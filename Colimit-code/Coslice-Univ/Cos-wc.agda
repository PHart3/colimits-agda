...
module _ {j} (A : Type j) where
  ...
  iso free-forg-adj-cos {a = U} {x = V} = free-forg-cos A ∘e eqv-aux
    where
      eqv-aux : (*[ Coprod U A , inr ] *→ V) ≃ (*[ Coprod U A , inr ] *→ *[ Lift (ty V) , lift ∘ fun V ])
      eqv-aux = equiv
        (λ (f , p) → (λ { (inl u) → lift (f (inl u)) ; (inr a) → lift (f (inr a)) }) , λ a → ap lift (p a))
        (λ (f , p) → (λ { (inl u) → lower (f (inl u)) ; (inr a) → lower (f (inr a)) }) , λ a → ap lower (p a))
