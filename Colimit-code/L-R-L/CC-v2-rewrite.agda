...
module _ {ℓv ℓe ℓ ℓd} {Γ : Graph ℓv ℓe} {A : Type ℓ} (F : CosDiag ℓd ℓ A Γ) {i j : Obj Γ} (g : Hom Γ i j) (a : A) where
  ...
  module _ {ℓc} (T : Coslice ℓc ℓ A) (f : P → ty T) (fₚ : (a : A) → f (left a)  == fun T a) where
    ...
    Ω-pth4 : ! (↯ (ap-seq (λ q → (! (ap (f ∘ right) (ap ψ (cglue g a)))) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) (ap-seq ! Ω))) ◃∎                                                        
      =ₛ ! (ap (λ q → ! (ap (f ∘ right) (ap ψ (cglue g a))) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) (ap ! (ap (λ q → q ∙ fₚ a) (ap (ap f) (E₃-v2 {f = left} {v = ψ} {u = right}          
           (λ x → ! (glue x)) (cglue g a) (id-βr g a)))))) ◃∙                                                                                                                               
         ! (ap (λ q → ! (ap (f ∘ right) (ap ψ (cglue g a))) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) (ap ! (ap (λ q → q ∙ fₚ a) (ap (ap f) (E₂-v2 {p = ap ψ (cglue g a)} (ψ-βr g a)        
           (! (glue (cin j a)))))))) ◃∙                                                                                                                                                     
         ! (ap (λ q → ! (ap (f ∘ right) (ap ψ (cglue g a))) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) (ap ! (ap (λ q → q ∙ fₚ a) (ap (ap f) (E₁-v2 {g = cin j} {R = cglue g (fun (F # i) a)}
           (snd (F <#> g) a)))))) ◃∙                                                                                                                                                        
         ! (ap (λ q → ! (ap (f ∘ right) (ap ψ (cglue g a))) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) (ap ! (ap-cp-revR f (right ∘ cin j) (snd (F <#> g) a) (ap right                      
           (cglue g (fun (F # i) a)))))) ◃∎                                                                                                                                                 
    ...
    abstract
      ...
      PathSeq2 : ! (ap (λ q → (! (ap (f ∘ right) (ap ψ (cglue g a)))) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) (ap ! (snd (comTri κ g) a))) ◃∎ =ₛ
            ! (ap (λ q → ! (ap (f ∘ right) (ap ψ (cglue g a))) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) (ap ! (ap (λ q → q ∙ fₚ a) (ap (ap f) (E₃-v2 {f = left} {v = ψ} {u = right}
              (λ x → ! (glue x)) (cglue g a) (id-βr g a)))))) ◃∙
            ! (ap (λ q → ! (ap (f ∘ right) (ap ψ (cglue g a))) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) (ap ! (ap (λ q → q ∙ fₚ a) (ap (ap f) (E₂-v2 {p = ap ψ (cglue g a)} (ψ-βr g a)
              (! (glue (cin j a)))))))) ◃∙
