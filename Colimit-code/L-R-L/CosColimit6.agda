...
open import CosColimit5

module CosColimit6 where
...
module Constr6 {ℓv ℓe ℓ ℓd ℓc} {Γ : Graph ℓv ℓe} {A : Type ℓ} (F : CosDiag ℓd ℓ A Γ) (T : Coslice ℓc ℓ A) where
  ...
  𝕣 (f , fₚ) i a = ap (λ p → (! (ap f (glue (cin i a))) ∙ fₚ a) ∙ p) (FPrecc-βr (PostComp ColCoC (f , fₚ)) (cin i a)) ◃∙ ap-inv-canc f (glue (cin i a)) (fₚ a) ◃∎
  ...
  module DiagCoher6 (i j : Obj Γ) (f : P → ty T) (fₚ : (a : A) → f (left a)  == fun T a) (g : Hom Γ i j) (a : A) where
    ...
    RLfunHtpy1 : transport (λ z → (! (ap f (glue z)) ∙ fₚ ([id] z)) ∙ ap (fst (RLfun (f , fₚ))) (glue z) == RfunEq (f , fₚ) (ψ z)) (cglue g a) 𝕣₂ ◃∎ =ₛ
    ...
    RLfunHtpy1 = transport (λ z → (! (ap f (glue z)) ∙ fₚ ([id] z)) ∙ ap (fst (RLfun (f , fₚ))) (glue z) == RfunEq (f , fₚ) (ψ z)) (cglue g a) 𝕣₂ ◃∎
                    =ₛ⟨ transp-id-concat (λ z → (! (ap f (glue z)) ∙ fₚ ([id] z)) ∙ ap (fst (RLfun (f , fₚ))) (glue z)) (λ z → RfunEq (f , fₚ) (ψ z)) (cglue g a)
                        ...
                        (λ z → (! (ap f (glue z)) ∙ fₚ ([id] z)) ∙ σ (comp K) (comTri K) z) (λ z → ap (λ p → (! (ap f (glue z)) ∙ fₚ ([id] z)) ∙ p) (FPrecc-βr K z)) (cglue g a)) ⟩
                  (ap (λ p → (! (ap f (glue (cin i a))) ∙ fₚ ([id] (cin i a))) ∙ p) (FPrecc-βr K (cin i a)) ∙
                    ! (apd-tr (λ z → (! (ap f (glue z)) ∙ fₚ ([id] z)) ∙ σ (comp K) (comTri K) z) (cglue g a)) ∙
                    ! (ap (transport (λ z → f (right (ψ z)) == fst (RLfun (f , fₚ)) (right (ψ z))) (cglue g a))
                      (ap (λ p → (! (ap f (glue (cin j (idf A a)))) ∙ fₚ ([id] (cin j (idf A a)))) ∙ p) (FPrecc-βr K (cin j (idf A a)))))) ◃∙
