...
module _ {ℓ₁ ℓ₂ ℓ₃} {A : Type ℓ₁} {B : Type ℓ₂} {C : Type ℓ₃} (f : A → B) (h : C → A) where
  ...
  transp-pth-cmpL : (g : C → B) {x y : C} (p : x == y) (q : f (h x) == g x) → transport (λ z → f (h z) == g z) p q == ! (ap f (ap h p)) ∙ q ∙ ap g p
  transp-pth-cmpL g idp q = ! (∙-unit-r q)
...
module _ {ℓ₁ ℓ₂ ℓ₃} {A : Type ℓ₁} {B : Type ℓ₂} {C : Type ℓ₃} (f : A → B) (m : B → C) where
  ...
  ap-∘-!-!-rid-rid idp idp p₂ p₅ p₆ = ap-!-!-!-rid m p₂ p₅ p₆ 
...
module ConstrMap {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ} (δ : CosDiagMor A F G) where
  ...
  module _ {i j : Obj Γ} (g : Hom Γ i j) (a : A) where
    ...
    ζ = transport (λ z → δ₀ (ψ₁ z) == ψ₂ z) (cglue g a) (ap (cin j) (snd (nat δ j) a))
        ...
        ! (! (ap (cin j) (comSq δ g (fun (F # i) a))) ∙ cglue g (fst (nat δ i) (fun (F # i) a))) ∙ ap (cin j ∘ (fst (nat δ j))) (snd (F <#> g) a) ∙ ap (cin j) (snd (nat δ j) a) ∙ ap ψ₂ (cglue g a) 
          =⟪ ap (λ p → ! p ∙ ap (cin j ∘ fst (nat δ j))
            (snd (F <#> g) a) ∙ ap (cin j) (snd (nat δ j) a) ∙ ap ψ₂ (cglue g a)) (ap (λ p → ! (ap (cin j) p) ∙ cglue g (fst (nat δ i) (fun (F # i) a))) (comSq-coher δ g a))  ⟫
  ...
  K : CosCocone A F (Cos P₂ left)
  fst (comp K i) = right ∘ cin i ∘ (fst (nat δ i))
  snd (comp K i) a = ap (right ∘ cin i) (snd (nat δ i) a) ∙ ! (glue (cin i a))
  fst (comTri K {j} {i} g) x = ap right (! (ap (cin j) (comSq δ g x)) ∙ cglue g (fst (nat δ i) x))
  snd (comTri K g) a = ↯ (Θ g a)
  ...
  span-mapFG : SpanMap-Rev SpCos₁ SpCos₂
  SpanMap-Rev.hA span-mapFG = idf A
  SpanMap-Rev.hB span-mapFG = δ₀
  SpanMap-Rev.hC span-mapFG = idf (Colim (ConsDiag Γ A))
  SpanMap-Rev.f-commutes span-mapFG = comm-sqr λ z → idp
  SpanMap-Rev.g-commutes span-mapFG = comm-sqr (λ z → ! (ℂ z))

  private
    module PM = PushoutMap span-mapFG
