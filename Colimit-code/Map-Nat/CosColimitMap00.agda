...
module ConstrMap {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ}
  ...
  ζ : {i j : Obj Γ} (g : Hom Γ i j) (a : A) →
    transport (λ z → δ₀ (ψ₁ z) == ψ₂ z) (cglue g a) (ap (cin j) (snd (nat δ j) a)) =-= ap (cin i) (snd (nat δ i) a)
  ζ {i} {j} g a =
    ...
    ! (ap δ₀ (! (ap (cin j) (snd (F <#> g) a)) ∙ cglue g (str (F # i) a))) ∙
    ap (cin j) (snd (nat δ j) a) ∙
    ap ψ₂ (cglue g a)
      =⟪ pre-cmp-!-!-∙ δ₀ (cin j) (snd (F <#> g) a) (cglue g (str (F # i) a))
           (ap (cin j) (snd (nat δ j) a) ∙ ap ψ₂ (cglue g a)) ⟫
    ! (ap δ₀ (cglue g (str (F # i) a))) ∙
    ap (cin j ∘ (fst (nat δ j))) (snd (F <#> g) a) ∙
    ap (cin j) (snd (nat δ j) a) ∙
    ap ψ₂ (cglue g a)
      =⟪ ap (λ p →
              ! p ∙ ap (cin j ∘ (fst (nat δ j))) (snd (F <#> g) a) ∙ ap (cin j) (snd (nat δ j) a) ∙ ap ψ₂ (cglue g a))
           (δ₀-βr g (str (F # i) a)) ⟫
    ...
    ap (cin j ∘ fst (nat δ j)) (snd (F <#> g) a) ∙ ap (cin j) (snd (nat δ j) a) ∙ ap ψ₂ (cglue g a)
      =⟪ ap (λ p →
           ! (! (ap (cin j)
                  (ap (fst (G <#> g)) (snd (nat δ i) a) ∙
                  snd (G <#> g) a ∙
  ...
  Θ {i} {j} g a =
      ...
      ! (ap right (! (ap (cin j)
          ...
          ! (snd (nat δ j) a) ∙
          ! (ap (fst (nat δ j)) (snd (F <#> g) a)))) ∙
        ap (cin j ∘ fst (G <#> g)) (snd (nat δ i) a) ∙ cglue g (str (G # i) a) ∙ ! (ap (cin i) (snd (nat δ i) a)))) ∙
      ap (right ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a) ∙
      ap (right ∘ cin j) (snd (nat δ j) a) ∙ ! (glue (cin j a))
  ...
  ℂ-β {i} {j} g a = =ₛ-in $
    ...
    (cglue-β
      ...
      (λ _ _ g a →  from-transp-g (λ z → δ₀ (ψ₁ z) == ψ₂ z) (cglue g a) (↯ (ζ g a))) g a) 
