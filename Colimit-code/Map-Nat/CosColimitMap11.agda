...
module ConstrMap12 {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ} (δ : CosDiagMor A F G) where
  ...
  module MapCoher11 {i j : Obj Γ} (g : Hom Γ i j) (a : A) where
    ...
    fib-coher-pre2 = 
      ...
      ap4-!-!-!-rid 𝕕₀ (snd (F <#> g) a) (cglue g (str (F # i) a)) (ap [id] (cglue g a)) (glue (cin j a)) ◃∙
      ap (λ p → ! (ap (right ∘ δ₀) (! (ap (cin j) (snd (F <#> g) a)) ∙ (cglue g (str (F # i) a)))) ∙ ! p ∙
           ap left (ap [id] (cglue g a))) (𝕕-βr (cin j a)) ◃∙
      ψ₁-free-aux (cglue g a) (snd (F <#> g) a) (cglue g (str (F # i) a)) ◃∙
      ap ! (ap (λ p → glue (cin i a) ∙ ap right (! p))
             (pre-cmp-!-!-∙ δ₀ (cin j) (snd (F <#> g) a)
               (cglue g (str (F # i) a)) (ap (cin j) (snd (nat δ j) a) ∙ ap ψ₂ (cglue g a)))) ◃∙
      ap ! (ap (λ p → glue (cin i a) ∙ ap right (! p))
             (ap (λ p → ! p ∙ ap (cin j ∘ fst (nat δ j)) (snd (F <#> g) a) ∙
             ap (cin j) (snd (nat δ j) a) ∙ ap ψ₂ (cglue g a)) (δ₀-βr g (str (F # i) a)))) ◃∙
      ap ! (ap (λ p → glue (cin i a) ∙ ap right (! p))
      ...
      ! (ap (λ p → ! (ap right p) ∙ ap (right ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a) ∙
              ap (right ∘ cin j) (snd (nat δ j) a) ∙ ! (glue (cin j a)))
              (ap (λ p → ! (ap (cin j) p) ∙ cglue g (fst (nat δ i) (str (F # i) a)))
                (comSq-coher δ g a))) ◃∎
        =ₛ⟨ 6 & 9 & id-red (id-βr g a) idp (snd (F <#> g) a) (cglue g (str (F # i) a)) (comSq-coher δ g a) (δ₀-βr g (str (F # i) a)) ⟩
      ! (ap (λ p → ! p ∙ ap (right ∘ cin j ∘ (fst (nat δ j))) (snd (F <#> g) a) ∙
              ap (right ∘ cin j) (snd (nat δ j) a) ∙ ! (glue (cin j a)))
            (
            ∘-ap 𝕕₀ right (cglue g (str (F # i) a)) ∙
