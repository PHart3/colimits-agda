  ...
  module MapCoher13 {i j : Obj Γ} (g : Hom Γ i j) (a : A) where
    ...
    fib-coher-pre4 = 
      ...
      ap (λ p → ! ((ap (right ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a) ∙ ((ap 𝕕₀ (! (glue (cin j a))) ∙ idp) ∙
           ! (ap 𝕕₀ (! (glue (cin j a))) ∙ idp)) ∙ ! (ap (right ∘ δ₀ ∘ cin j) (snd (F <#> g) a))) ∙ p) ∙
           ap (right ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a) ∙ ap (right ∘ cin j) (snd (nat δ j) a) ∙ ! (glue (cin j a)))
         (∘-ap 𝕕₀ right (cglue g (str (F # i) a))) ◃∙
      ...
      ap (λ p → ! (! (ap (right ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a)) ∙ p) ∙
         ...
         (ap-∘ right δ₀ (cglue g (str (F # i) a))) ◃∙
      ap (λ p →  ! (! (ap (right ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a)) ∙ ap right p) ∙ ! (glue (cin j a) ∙
           ap right (! (ap (cin j) (snd (nat δ j) a))))) (δ₀-βr g (str (F # i) a)) ◃∙
      ↯ (ap2-!5-2 right (cin j) (cglue g (fst (nat δ i) (str (F # i) a)))
          (ap (cin j) (comSq δ g (str (F # i) a))) (snd (nat δ j) a)
    ...
    fib-coher-pre5 =
      ...
      ↯ (ap2-!5-2 right (cin j) (cglue g (fst (nat δ i) (str (F # i) a)))
          (ap (cin j) (comSq δ g (str (F # i) a))) (snd (nat δ j) a)
          (ap (right {d = SpCos₂} ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a)) (glue (cin j a))) ◃∎
        =ₛ⟨ 0 & 6 & δ₀-red (δ₀-βr g (str (F # i) a)) ⟩
      ↯ (δ₀-free (! (ap (cin j) (comSq δ g (str (F # i) a))) ∙ cglue g (fst (nat δ i) (str (F # i) a)))
          (snd (nat δ j) a) (glue (cin j a)) (ap (right ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a))) ◃∙
      ↯ (ap2-!5-2 right (cin j) (cglue g (fst (nat δ i) (str (F # i) a))) (ap (cin j) (comSq δ g (str (F # i) a)))
          (snd (nat δ j) a) (ap (right {d = SpCos₂} ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a)) (glue (cin j a))) ◃∎
        =ₛ⟨ δ₀-comSq-red (cglue g (fst (nat δ i) (str (F # i) a))) (ap (cin j) (comSq δ g (str (F # i) a)))
