...
module _ {ℓ₁ ℓ₂} {A : Type ℓ₁} {B : Type ℓ₂} {f g : A → B} where
  ...
  hmtpy-nat-! : (H : f ∼ g) {x y : A} (p : x == y) → ! (H x) == ap g p ∙ ! (H y) ∙ ! (ap f p)
  hmtpy-nat-! H {x = x} idp = ! (∙-unit-r (! (H x)))
...
module _ {ℓ₀ ℓ₁ ℓ₂ ℓ₃} {A₁ : Type ℓ₀} {A₂ : Type ℓ₁} {B : Type ℓ₂} {C : Type ℓ₃} (f : B → C) (h : A₂ → B) (g : A₁ → B) where
  ...
  long-path-red-V : {c₁ c₂ : C} (p₁ : c₁ == c₂) {a₁ a₂ : A₂} (p₂ : a₁ == a₂) (p₃ : c₂ == f (h a₂))
    ...
    → (p₁ ∙ p₃ ∙ ! (ap (f ∘ h) p₂)) ∙ ap f (ap h p₂ ∙ p₄ ∙ ! p₅ ∙ ! (ap g p₆)) ∙ ap (f ∘ g) p₆ ∙ ap f p₅ ∙ p₇ == p₁ ∙ p₃ ∙ ap f p₄ ∙ p₇
...
-- p₁ = (comSq δ g x)
-- p₂ = (cglue g (fst (nat δ i) x))

module ConstrMap23 {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ} (δ : CosDiagMor A F G) where
    ...
    λ {j} {i} g → (λ x → ! (ap (fst (comp₁ j)) (comSq δ g x)) ∙ fst (comTri₁ g) (fst (nat δ i) x)) , λ a → ↯ (V g a)
      where
        V : {j i : Obj Γ} (g : Hom Γ i j) (a : A) →
          ! (! (ap (fst (comp₁ j)) (comSq δ g (fun (F # i) a))) ∙ fst (comTri₁ g) (fst (nat δ i) (fun (F # i) a))) ∙ snd (< A > < A > comp₁ j ∘ nat δ j ∘ F <#> g) a
          =-= snd (< A > comp₁ i ∘ nat δ i) a
        V {j} {i} g a =
          ! (! (ap (fst (comp₁ j)) (comSq δ g (fun (F # i) a))) ∙ fst (comTri₁ g) (fst (nat δ i) (fun (F # i) a))) ∙ snd (< A > < A > comp₁ j ∘ nat δ j ∘ F <#> g) a
            =⟪ !-!-∙-pth (ap (fst (comp₁ j)) (comSq δ g (fun (F # i) a))) (fst (comTri₁ g) (fst (nat δ i) (fun (F # i) a)))   ⟫
          ! (fst (comTri₁ g) (fst (nat δ i) (fun (F # i) a))) ∙ ap (fst (comp₁ j)) (comSq δ g (fun (F # i) a)) ∙ snd (< A > < A > comp₁ j ∘ nat δ j ∘ F <#> g) a
            =⟪ ap (λ p → ! (fst (comTri₁ g) (fst (nat δ i) (fun (F # i) a))) ∙ ap (fst (comp₁ j)) p ∙ snd (< A > < A > comp₁ j ∘ nat δ j ∘ F <#> g) a) (comSq-coher δ g a) ⟫
          ...
          snd (< A > < A > comp₁ j ∘ nat δ j ∘ F <#> g) a
            =⟪ long-path-red-V  (fst (comp₁ j)) (fst (G <#> g)) (fst (nat δ j)) (ap (fst (comp₁ i)) (snd (nat δ i) a)) (snd (nat δ i) a)
                 (! (fst (comTri₁ g) (fun (G # i) a))) (snd (G <#> g) a) (snd (F <#> g) a) (snd (nat δ j) a) (snd (comp₁ j) a)  ⟫
  ...
  module _ {ℓc} (T : Coslice ℓc ℓ A) (f : P₂ → ty T) (fₚ : (a : A) → f (left a) == fun T a) where
    ...
    module _ {i j : Obj Γ} (g : Hom Γ i j) (a : A) where
      ...
      NatSq2-Λ-coher-aux3 : {w y : ty (G # i)} (τ₁₀ : w == y) {z : ty (F # j)} (τ₁₃ : fst (G <#> g) y == fst (nat δ j) z)
        →
        ...
        ap-∘-∙-s f (right ∘ (λ v → cin j (fst (G <#> g) v))) τ₁₀ (ap (right ∘ cin j) τ₁₃ ∙ idp)
          ==
      ...
      NatSq2-Λ-coher-aux2 : (τ₁₀ : fst (nat δ i) (fun (F # i) a) == fun (G # i) a) {x : ty (F # j)} (τ₁₃ : fst (G <#> g) (fun (G # i) a) == fst (nat δ j) x) 
        →
        ...
        ap-∘-∙-s f (right ∘ cin i) τ₁₀ (! (ap right (cglue g (fun (G # i) a))) ∙ ap (right ∘ cin j) τ₁₃ ∙ idp)
          ==
      ...
      NatSq2-Λ-coher-aux2 τ₁₀ τ₁₃ = IndFunHom {P = λ h H →
        ...
        ap (λ q → q) (ap-cp-revR f (right ∘ cin j ∘ fst (nat δ j)) idp
          ...
          idp (H (fun (G # i) a)) ∙ idp)) ∙
        ap-∘-∙-s f (right ∘ h) τ₁₀ (! (ap right (H (fun (G # i) a))) ∙ ap (right ∘ cin j) τ₁₃ ∙ idp)
          ==
        !-!-∙-pth (ap (λ x → f (right (cin j x))) (ap (fst (G <#> g)) τ₁₀ ∙ τ₁₃ ∙ idp))
      ...
      NatSq2-Λ-coher-aux : (τ₁₀ : fst (nat δ i) (fun (F # i) a) == fun (G # i) a) (τ₁₃ : fst (G <#> g) (fun (G # i) a) == fst (nat δ j) (fst (F <#> g) (fun (F # i) a)))
        {σ₁ : fst (G <#> g) (fst (nat δ i) (fun (F # i) a)) == fst (nat δ j) (fst (F <#> g) (fun (F # i) a))} (τ₁₄ : σ₁ == ap (fst (G <#> g)) τ₁₀ ∙ τ₁₃ ∙ idp)
        →
        ! (ap (λ p → ! p ∙ idp) (ap-∘-∘-!-∙-rid f right (cin j) σ₁
          (cglue g (fst (nat δ i) (fun (F # i) a))))) ∙
        long-path-red {f = f ∘ right ∘ cin j ∘ fst (nat δ j)} {g = f ∘ right ∘ cin j ∘ fst (nat δ j)} idp idp idp (ap f (ap right (! (ap (cin j) σ₁) ∙
        cglue g (fst (nat δ i) (fun (F # i) a))))) idp ∙
        ap (λ q → q) (ap-cp-revR f (right ∘ cin j ∘ fst (nat δ j)) idp
          (ap right (! (ap (cin j) σ₁) ∙
        cglue g (fst (nat δ i) (fun (F # i) a)))) ∙
        ...
        ap-∘-∙-s f (right ∘ cin i) τ₁₀ (! (ap right (cglue g (fun (G # i) a))) ∙
        ...
        long-path-red-V (λ x → f (right (cin j x))) (fst (G <#> g))
          (fst (nat δ j)) (ap (λ x → f (right (cin i x))) τ₁₀) τ₁₀
          (! (ap f (ap right (cglue g (fun (G # i) a))))) τ₁₃ idp idp idp ∙
        ap (_∙_ (ap (λ x → f (right (cin i x))) τ₁₀)) (ap-cp-revR f (λ x → right (cin j x)) τ₁₃ (ap right (cglue g (fun (G # i) a))) ∙ idp)
      NatSq2-Λ-coher-aux τ₁₀ τ₁₃ idp = NatSq2-Λ-coher-aux2 τ₁₀ τ₁₃

-- σ₁ = (comSq δ g (fun (F # i) a))

      NatSq2-Λ-coher : {x : ty (F # j)} (τ₅ : fst (F <#> g) (fun (F # i) a) == x) {y : ty (G # j)} (τ₆ : fst (nat δ j) x == y) {z : P₂} (τ₇ : right (cin j y) == z)
        {w : ty T} (τ₈ : f z == w) (τ₁₀ : fst (nat δ i) (fun (F # i) a) == fun (G # i) a) (τ₁₃ : fst (G <#> g) (fun (G # i) a) == y)
        (τ₁₄ : comSq δ g (fun (F # i) a) == (ap (fst (G <#> g)) τ₁₀ ∙ τ₁₃ ∙ ! τ₆ ∙ ! (ap (fst (nat δ j)) τ₅)))
        {τ₁₁ : right (cin i (fun (G # i) a)) == z} (τ₁ : ! (ap right (cglue g (fun (G # i) a))) ∙ ap (right ∘ cin j) τ₁₃ ∙ τ₇ == τ₁₁) 
        → 
...
-}
    ...
    CosColim-NatSq2 : CosCocEq F T (Map-to-Lim-map F (f , fₚ) K) (Diag-to-Lim-map (PostComp ColCoC (f , fₚ)))
    W CosColim-NatSq2 i x = idp
    u CosColim-NatSq2 i a = ap-∘-∙-s f (right ∘ cin i) (snd (nat δ i) a) (! (glue (cin i a)))
    ...
    CosColim-NatSq2-eq : Map-to-Lim-map F (f , fₚ) K == Diag-to-Lim-map (PostComp ColCoC (f , fₚ))
    CosColim-NatSq2-eq = CosCocEq-ind F T (Map-to-Lim-map F (f , fₚ) K) (CosColim-NatSq2)
