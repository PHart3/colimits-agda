...
module _ {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ} (δ : CosDiagMor A F G) where
  ...
  module _ {ℓc} {T : Coslice ℓc ℓ A} where
    ...
    module _ (f : P₂ → ty T) (fₚ : (a : A) → f (left a) == fun T a) where 
      ...
      NatSq-PreCmp1 =
        Diag-to-Lim-map δ (PostComp-cos (ColCoC G) (f , fₚ))
          =⟨ ! (CosColim-NatSq2-eq T f fₚ) ⟩
        Map-to-Lim-map F (f , fₚ) K-diag
          =⟨ ap (Map-to-Lim-map F (f , fₚ)) (! (LRfunEq K-diag)) ⟩
        Map-to-Lim-map F (f , fₚ) (PostComp-cos (ColCoC F) (recCosCoc K-diag))
          =⟨ CosColim-NatSq1-eq F (f , fₚ) (fst (recCosCoc K-diag)) (snd (recCosCoc K-diag)) ⟩
        PostComp-cos (ColCoC F) (f , fₚ ∘* recCosCoc K-diag) =∎

      NatSq-PreCmp2 : PostComp-cos (ColCoC F) (f , fₚ ∘* recCosCoc K-diag) == PostComp-cos (ColCoC F) (f , fₚ ∘* 𝕕)
      NatSq-PreCmp2 = ap (λ h → PostComp-cos (ColCoC F) (f , fₚ ∘* h)) (K-diag-𝕕-eq δ)

    NatSq-PreCmp : (f* : (Cos P₂ left) *→ T) → Diag-to-Lim-map δ (PostComp-cos (ColCoC G) f*) == PostComp-cos (ColCoC F) (f* ∘* 𝕕)
