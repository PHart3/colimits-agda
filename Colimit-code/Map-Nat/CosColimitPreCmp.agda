...
-- naturality square arising from precomposition of A-diagram map
...
module _ {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ}
  ...
  module _ {ℓc} {T : Coslice ℓc ℓ A} where
    ...
    module _ (f : P₂ → ty T) (fₚ : (a : A) → f (left a) == str T a) where 
      ...
      NatSq-PreCmp2 :
        PostComp-cos (ColCoC-cos F) (f , fₚ ∘* recCosCoc CC-from-diagmap) == PostComp-cos (ColCoC-cos F) (f , fₚ ∘* 𝕕)
      NatSq-PreCmp2 = ap (λ h → PostComp-cos (ColCoC-cos F) (f , fₚ ∘* h)) (CC-from-diagmap-𝕕-eq δ)

    NatSq-PreCmp : (f* : (Cos P₂ left) *→ T) →
      Diag-to-Lim-map δ (PostComp-cos (ColCoC-cos G) f*) == PostComp-cos (ColCoC-cos F) (f* ∘* 𝕕)
