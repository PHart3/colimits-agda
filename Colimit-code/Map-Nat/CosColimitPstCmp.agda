...
module _ {ℓv ℓe ℓ ℓd ℓc₁ ℓc₂} {Γ : Graph ℓv ℓe} {A : Type ℓ} (F : CosDiag ℓd ℓ A Γ)
  {T : Coslice ℓc₁ ℓ A} {U : Coslice ℓc₂ ℓ A} (φ : < A > T *→ U) where
  ...
  comTri (Map-to-Lim-map (comp₁ & comTri₁)) {j} {i} =
    ...
    λ a →
      !-ap-ap-∘-ap-∙ φ₁ (fst (comp₁ j)) (snd (F <#> g) a) {r = snd (comp₁ j) a} {s = snd φ a}
  ...
  module _ (f : P → ty T) (fₚ : (a : A) → f (left a) == str T a) where
    ...
    CosColim-NatSq1 :
      CosCocEq (Map-to-Lim-map (PostComp-cos ColCoC-cos (f , fₚ))) (PostComp-cos ColCoC-cos (φ ∘* (f , fₚ)))
      ...
      where
        lemma : (a : A) → 
          ! (ap (λ p → ! p ∙  ap (φ₁ ∘ f ∘ fst (comp ColCoC-cos j)) (snd (F <#> g) a) ∙
              ...
              (∘-ap φ₁ f (ap right (cglue g (str (F # i) a))))) ◃∙
          ap (λ p → ! (p ∙ ap φ₁ (ap f (fst (comTri ColCoC-cos g) (str (F # i) a)))) ∙
          ...
          ap (λ p → ! ((ap (φ₁ ∘ f ∘ fst (comp ColCoC-cos j)) (snd (F <#> g) a) ∙
              ...
              ! (ap (fst (φ ∘* f ∘ fst (comp ColCoC-cos j) , (λ a₁ → ap f (snd (comp ColCoC-cos j) a₁) ∙ fₚ a₁)))
                  (snd (F <#> g) a))) ∙
              ap φ₁ (ap f (fst (comTri ColCoC-cos g) (str (F # i) a)))) ∙
              ap (φ₁ ∘ f ∘ fst (comp ColCoC-cos j)) (snd (F <#> g) a) ∙
              ap (φ₁ ∘ f) (snd (comp ColCoC-cos j) a) ∙ snd (φ ∘* f , fₚ) a)
          ...
          ap (λ q → q)
            (!-ap-ap-∘-ap-∙ φ₁ (f ∘ fst (comp ColCoC-cos j)) (snd (F <#> g) a)
              (ap f (fst (comTri ColCoC-cos g) (str (F # i) a))) ∙
              ap (λ p → ap φ₁ p ∙ φ₂ a)
            (!-ap-ap-∘-ap-∙ f (fst (comp ColCoC-cos j)) (snd (F <#> g) a) (fst (comTri ColCoC-cos g) (str (F # i) a)) ∙
            ap (λ p → p ∙ fₚ a) (ap (ap f) (snd (comTri ColCoC-cos g) a)))) ◃∙
          ap-∘-∙-∙ φ₁ f (! (glue (cin i a))) (fₚ a) ◃∎
            =ₛ
          (!-ap-ap-∘-ap-∙ (φ₁ ∘ f) (fst (comp ColCoC-cos j)) (snd (F <#> g) a)
            (fst (comTri ColCoC-cos g) (str (F # i) a)) ∙
        ...
        lemma a =
          ...
          ap (λ q → q)
            ...
            ap (λ p → p ∙ fₚ a) (ap (ap f) (snd (comTri ColCoC-cos g) a)))) ◃∙
          ap-∘-∙-∙ φ₁ f (! (glue (cin i a))) (fₚ a) ◃∎
            =ₛ⟨ 0 & 4 & NatSq-1-Λ-red g a (ap right (cglue g (str (F # i) a))) (snd (F <#> g) a)
                          (! (glue (cin j a))) (fₚ a) (φ₂ a) ⟩
          ↯ (NatSq-1-Λ-aux g a (ap right (cglue g (str (F # i) a))) (snd (F <#> g) a)
            (! (glue (cin j a))) (fₚ a) (φ₂ a)) ◃∙ 
          ap (λ q → q)
            (!-ap-ap-∘-ap-∙ φ₁ (f ∘ fst (comp ColCoC-cos j)) (snd (F <#> g) a)
              (ap f (fst (comTri ColCoC-cos g) (str (F # i) a))) ∙
            ap (λ p → ap φ₁ p ∙ φ₂ a)
              ...
              ap (λ p → p ∙ fₚ a) (ap (ap f) (snd (comTri ColCoC-cos g) a)))) ◃∙
          ap-∘-∙-∙ φ₁ f (! (glue (cin i a))) (fₚ a) ◃∎
            =ₛ⟨ NatSq-1-Λ-red2 g a (ap right (cglue g (str (F # i) a))) (snd (F <#> g) a) (! (glue (cin j a)))
                  (snd (comTri ColCoC-cos g) a) ⟩          
          (!-ap-ap-∘-ap-∙ (φ₁ ∘ f) (fst (comp ColCoC-cos j)) (snd (F <#> g) a)
            (fst (comTri ColCoC-cos g) (str (F # i) a)) ∙
          ap (λ p → p ∙ snd (φ ∘* f , fₚ) a) (ap (ap (φ₁ ∘ f)) (snd (comTri ColCoC-cos g) a))) ◃∎ ∎ₛ

    CosColim-NatSq1-== : Map-to-Lim-map (PostComp-cos ColCoC-cos (f , fₚ)) == PostComp-cos ColCoC-cos (φ ∘* (f , fₚ))
    CosColim-NatSq1-== = CosCocEq-to-== (CosColim-NatSq1)
