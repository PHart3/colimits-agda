...
module ConstrE2 {ℓv ℓe ℓ ℓd ℓc} {Γ : Graph ℓv ℓe} {A : Type ℓ} (F : CosDiag ℓd ℓ A Γ) (T : Coslice ℓc ℓ A) (K : CosCocone A F T) where
  ...
  abstract
    ...
    FPrecc-transf : (i j : Obj Γ) (g : Hom Γ i j) (a : A) → ap-inv-rid (fst (recCosCoc K)) (glue (cin i a)) ◃∙ ap ! (FPrecc-βr K (cin i a)) ◃∎ =ₛ
      ! (apd-tr (λ z → ap (fst (recCosCoc K)) (! (glue z)) ∙ idp) (cglue g a)) ◃∙ ap (transport (λ z → reccForg K (ψ z) == fun T ([id] z)) (cglue g a))
        (ap-inv-rid (fst (recCosCoc K)) (glue (cin j a)) ∙ ap ! (FPrecc-βr K (cin j a))) ◃∙ tranp-inv-comm (fun T ∘ [id]) (reccForg K ∘ ψ) (cglue g a) (! (snd (comp K j) a)) ◃∙
      ap-seq ! (η (comp K) (comTri K) i j g a)
    FPrecc-transf i j g a = ap-inv-rid (fst (recCosCoc K)) (glue (cin i a)) ◃∙ ap ! (FPrecc-βr K (cin i a)) ◃∎
                              =ₛ⟨ =ₛ-in (apd-tr-coher (λ z → ap (fst (recCosCoc K)) (! (glue z)) ∙ idp) (λ z → ! (σ (comp K) (comTri K) z)) (cglue g a)
                                (λ z →  ap-inv-rid (fst (recCosCoc K)) (glue z) ∙ ap ! (FPrecc-βr K z))) ⟩
                            ! (apd-tr (λ z → ap (fst (recCosCoc K)) (! (glue z)) ∙ idp) (cglue g a)) ◃∙ ap (transport (λ z → reccForg K (ψ z) == fun T ([id] z)) (cglue g a))
                              (ap-inv-rid (fst (recCosCoc K)) (glue (cin j a)) ∙ ap ! (FPrecc-βr K (cin j a))) ◃∙ apd-tr (λ z → ! (σ (comp K) (comTri K) z)) (cglue g a) ◃∎
  ...
  module Equiv2a (i j : Obj Γ) (g : Hom Γ i j) (a : A) where
    ...
    Ξ-inst = ap (λ p → ! (p ∙  fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙ ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a)
    ...
    Ξ-RW2 =       ap (λ p → ! (p ∙  fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙ ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a)
                    (hmtpy-nat-rev (λ z → idp) (snd (F <#> g) a) (snd (comp LRfun j) a)) ◃∙
                  ap (λ p → ! ((ap (fst (comp K j)) (snd (F <#> g) a) ∙ (p ∙ ! (snd (comp LRfun j) a)) ∙ ! (ap (fst (comp LRfun j)) (snd (F <#> g) a))) ∙
                    fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙
                    ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a) (ap-inv-rid (fst (recCosCoc K)) (glue (cin j a))) ◃∙
    ...
    Ξ-RW3 =       ap (λ p → ! (p ∙  fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙ ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a)
                    (hmtpy-nat-rev (λ z → idp) (snd (F <#> g) a) (snd (comp LRfun j) a)) ◃∙
                  ap (λ p → ! ((ap (fst (comp K j)) (snd (F <#> g) a) ∙ (p ∙ ! (snd (comp LRfun j) a)) ∙ ! (ap (fst (comp LRfun j)) (snd (F <#> g) a))) ∙
                    fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙
                    ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a) (ap-inv-rid (fst (recCosCoc K)) (glue (cin j a))) ◃∙
    ...
    Ξ-RW4 =       ap (λ p → ! (p ∙  fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙ ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a)
                    (hmtpy-nat-rev (λ z → idp) (snd (F <#> g) a) (snd (comp LRfun j) a)) ◃∙
                  ap (λ p → ! ((ap (fst (comp K j)) (snd (F <#> g) a) ∙ (p ∙ ! (snd (comp LRfun j) a)) ∙ ! (ap (fst (comp LRfun j)) (snd (F <#> g) a))) ∙
                    fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙
                    ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a) (ap-inv-rid (fst (recCosCoc K)) (glue (cin j a))) ◃∙
    ...
    Ξ-RW5 =       ap (λ p → ! (p ∙  fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙ ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a)
                    (hmtpy-nat-rev (λ z → idp) (snd (F <#> g) a) (snd (comp LRfun j) a)) ◃∙
                  ap (λ p → ! ((ap (fst (comp K j)) (snd (F <#> g) a) ∙ (p ∙ ! (snd (comp LRfun j) a)) ∙ ! (ap (fst (comp LRfun j)) (snd (F <#> g) a))) ∙
                    fst (comTri LRfun g) (fun (F # i) a) ∙ idp) ∙
                    ap (fst (comp K j)) (snd (F <#> g) a) ∙ snd (comp K j) a) (ap-inv-rid (fst (recCosCoc K)) (glue (cin j a))) ◃∙
