...
module _ {ℓv ℓe ℓd} {Γ : Graph ℓv ℓe} where
  ...
  module CosColContr (F : Diagram Γ (Coslice-wc Unit (lmax (lmax ℓv ℓe) ℓd))) where
    ...
    abstract
      ...
      Cos-diag-contr-col-contr : (∀ i → is-contr (ty (Δ # i))) → {X : Coslice (lmax (lmax ℓv ℓe) ℓd) _ Unit} {K : Cocone-wc F X} → is-colim K → is-contr (ty X)
      Cos-diag-contr-col-contr cs {X} cK = equiv-preserves-level ty-≃ {{diag-contr-po-contr cs}}
        where abstract
          ...
          ty-≃ = (_ , eqv-wc-to-eqv-ty (F-equiv-wc (Forg-funct-cos Unit)
            (col-wc-unq (pentagon-wc-Cos Unit) (triangle-wc-Cos Unit) (ColCoc-is-colim {ℓd = ℓd} F) cK)))

  module PtdColContr (F : Diagram Γ (Ptd-wc (lmax (lmax ℓv ℓe) ℓd))) {X : Ptd (lmax (lmax ℓv ℓe) ℓd)} {K : Cocone-wc F X} (cK : is-colim K) where

    private iso-F = fst (fst (Ptd-Cos-iso {lmax (lmax ℓv ℓe) ℓd}))

    open CosColContr (F-diag iso-F F)

    abstract
      ptdcolim-contr : (∀ i → is-contr (de⊙ (D₀ F i))) → is-contr (de⊙ X)
      ptdcolim-contr cs = Cos-diag-contr-col-contr cs (wc-iso-colim-prsrv (Ptd-Cos-iso {lmax (lmax ℓv ℓe) ℓd}) cK)
