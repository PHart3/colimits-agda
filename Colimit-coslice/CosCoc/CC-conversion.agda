...
module _ {i j} {A : Type j} {Γ : Graph ℓv ℓe} {Δ : Diagram Γ (Coslice-wc A i)} where
  ...
  CosCoc-wc-coher {K = K} f = coc-to-== Γ ((λ _ → idp) , (λ {x} {y} g →
    ap (λ p → UndHom∼-to-== (*→-assoc f (comp K y) (D₁ Δ g)) ∙ p) (whisk-cos-conv-l (comTri K g)) ∙
...
module _ {i j} {A : Type j} {Γ : Graph ℓv ℓe} {Δ : Diagram Γ (Coslice-wc A (lmax i j))} where
  ...
  snd (forg-coc-mor-cos {K₁ = K₁} (f , σ)) = ! (ap (F-coc (Forg-funct-cos A {i})) (! σ) ∙
    coc-to-== Γ ((λ _ → idp) ,
      (λ {x} {y} g →
        ap-∙ fst (UndHom∼-to-== (*→-assoc f (leg K₁ y) (D₁ Δ g))) (ap (λ m → f ∘* m) (tri K₁ g)) ∙
        ap (λ p → p ∙ ap (λ r → fst r) (ap (λ m → f ∘* m) (tri K₁ g))) (! (fst=-UndHom∼ (*→-assoc f (leg K₁ y) (D₁ Δ g))) ∙
          ! (λ=-η idp)) ∙
  ...
  abstract
    ...
    CocForg-coh K = coc-to-== Γ ((λ _ → idp) , λ g →
      fst=-UndHom∼  (UndHom∼-from-== (tri K g)) ∙ ap (ap fst) (<–-inv-l UndHom-∼-==-≃ (tri K g)))
