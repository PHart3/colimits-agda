...
module Constr4 {ℓv ℓe ℓ ℓd ℓc} {Γ : Graph ℓv ℓe} {A : Type ℓ} (F : CosDiag ℓd ℓ A Γ) (T : Coslice ℓc ℓ A) where
  ...
  module DiagCoher4 (i j : Obj Γ) (f : po-coscol-tip → ty T) (fₚ : (a : A) → f (left a)  == str T a) (g : Hom Γ i j) (a : A) where
    ...
    PathSeq1 :
      (! (ap f (glue (cin i a))) ∙ fₚ a) ∙ ! (ap f (! (glue (cin i a))) ∙ fₚ a)
        =-=
      transport (λ x → f (right (ψ x)) == f (right (ψ x))) (cglue g a)
    ...
    abstract
      ...
      CommSq1 idp idp = =ₛ-in (lemma (glue (cin j a)) (fₚ a))
          where
            lemma : {x : po-coscol-tip} (r : left a == x) {y : ty T} (s : f (left a) == y) →
              ! (ap (_∙_ (! (ap f r) ∙ s)) (! (∙-unit-r (! (ap f (! r) ∙ s))))) ∙ ! (∙-unit-r ((! (ap f r) ∙ s) ∙ ! (ap f (! r) ∙ s)))
                ==
              ! (∙-assoc (! (ap f r) ∙ s) (! (ap f (! r) ∙ s)) idp)
            lemma idp idp = idp 

      Reduce2 : {x : Colim (ConsDiag Γ A)} (p : cin j a == x)
        {R : (reccForg K) (cin j (fst (F <#> g) (str (F # i) a))) == (reccForg K) (ψ x)} →
        (! (ap f (glue x)) ∙ fₚ ([id] x)) ∙
        ! (ap (str T) (ap [id] p)) ∙
      ...
      CommSq2 : {x : Colim (ConsDiag Γ A)} (p : cin j a == x) {s : cin j (fst (F <#> g) (str (F # i) a))  == ψ x}
        ...
        ! (ap (λ q → (! (ap f (glue x)) ∙ fₚ ([id] x)) ∙ q)
            (H₂ (snd (F <#> g) a) (ap f (! (glue (cin j a))) ∙ fₚ a) {p = ! (ap (str T) (ap [id] p))} s V)) ◃∙
        Reduce1 p ◃∙
        O₂
          {p = ! (ap (f ∘ right) (ap ψ p))} {g = cin j}
          {q = ((! (ap f (glue (cin j a))) ∙ fₚ a) ∙ ! (ap f (! (glue (cin j a))) ∙ fₚ a))}
          (snd (F <#> g) a) (ap f (! (glue (cin j a))) ∙ fₚ a) s V ◃∎
          =ₛ
        Reduce2 p {R = R} ◃∎
      CommSq2 idp {s = s} {R = R} idp = lemma (fₚ a) (snd (F <#> g) a) (glue (cin j a)) 
          where
            lemma : {y : ty T} (w : f (left a) == y) {z : ty (F # j)} (t : fst (F <#> g) (str (F # i) a) == z)
              (r : left a == right (cin j z)) →
            ...
            lemma idp idp r = lemma2 r
              where
                lemma2 : {x : po-coscol-tip} (e : x == right (cin j (fst (F <#> g) (str (F # i) a))))
                  →
      ...
      Reduce3 : {x : Colim (ConsDiag Γ A)} (p : cin j a == x) {t : ty T} (V : str T a == t) →
        (! (ap f (glue x)) ∙ fₚ ([id] x)) ∙ ! (ap (str T) (ap [id] p)) ∙ V
          ==
        ! (ap (f ∘ right) (ap ψ p)) ∙
        ((! (ap f (glue (cin j a))) ∙ fₚ a) ∙ ! (ap f (! (glue (cin j a))) ∙ fₚ a)) ∙ (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ V
      Reduce3 idp V = R3lemma (glue (cin j a)) (fₚ a) V 
          module _ where
            R3lemma : {x : po-coscol-tip} (r : left a == x) {y : ty T} (s : f (left a) == y) {z : ty T} (w : y == z)
              → (! (ap f r) ∙ s) ∙ w == ((! (ap f r) ∙ s) ∙ ! ((ap f (! r)) ∙ s)) ∙ (ap f (! r) ∙ s) ∙ w
            R3lemma idp idp w = idp

      CommSq3 : {x : Colim (ConsDiag Γ A)} (p : cin j a == x)
        {R : (reccForg K) (cin j (fst (F <#> g) (str (F # i) a))) == (reccForg K) (ψ x)}
        {S : str T a  == (reccForg K) (ψ x)}
        (E : ! (! R ∙ ap (f ∘ right ∘ cin j) (snd (F <#> g) a) ∙ ap f (! (glue (cin j a))) ∙ fₚ a) == S) →
        ! (ap (λ q →  (! (ap f (glue x)) ∙ fₚ ([id] x)) ∙ q) (ap (λ q → ! (ap (str T) (ap [id] p)) ∙ q) E)) ◃∙
        Reduce2 p {R = R} ◃∙
        ap (λ q → ! (ap (f ∘ right) (ap ψ p)) ∙
        ((! (ap f (glue (cin j a))) ∙ fₚ a) ∙ ! (ap f (! (glue (cin j a))) ∙ fₚ a)) ∙
        (ap f (! (glue (cin j a))) ∙ fₚ a) ∙ q) E ◃∎
          =ₛ
        Reduce3 p S ◃∎
      CommSq3 idp {R = R} idp = lemma (glue (cin j a)) (fₚ a)
        (! (! R ∙ ap (f ∘ right ∘ cin j) (snd (F <#> g) a) ∙ ap f (! (glue (cin j a))) ∙ fₚ a))
        where
          lemma : {x : po-coscol-tip} (r : left a == x) {y : ty T} (s : f (left a) == y) {z : ty T} (w : y == z) →
      ...
      Reduce4 idp {z = z} e u = R4lemma (glue (cin j a)) (fₚ a) (! (ap (str T) e)) (! u)
        module _ where
          R4lemma : {x n : po-coscol-tip} (r : n == x) {y d c : ty T} (s : f n == y) (q : y == d) (U : d == c)
      ...
      CommSq4 idp idp u = lemma (glue (cin j a)) (fₚ a)
        where
          lemma : {x : po-coscol-tip} (r : left a == x) (s : f (left a) == str T a) →
            idp ◃∙
            R3lemma (! u) r s (! u) ◃∙
            ap (λ q →  ((! (ap f r) ∙ s) ∙ ! (ap f (! r) ∙ s)) ∙ q ∙ ! u) (! (∙-unit-r (ap f (! r) ∙ s))) ◃∎
              =ₛ
      ...
      CommSq5 : {x : Colim (ConsDiag Γ A)} (p : cin j a == x) →
        ...
        O₅ {f = f ∘ right} ((! (ap f (glue (cin j a))) ∙ fₚ a) ∙ ! (ap f (! (glue (cin j a))) ∙ fₚ a)) p
          (ap f (! (glue x)) ∙ fₚ ([id] x)) ◃∎
          =ₛ
        ! (apd-tr (λ x → (! (ap f (glue x)) ∙ fₚ ([id] x)) ∙ ! (ap f (! (glue x)) ∙ fₚ ([id] x))) p) ◃∎
      CommSq5 idp = lemma (glue (cin j a)) (fₚ a)
        where
          lemma : {n : po-coscol-tip} (r : n == right (ψ (cin j a))) {t : ty T} (s : f n == t)
            →
      ...
      BigReduct1 =
        ...
        (range 3 2 (transpEq-◃ ((! (ap f (glue (cin j a))) ∙ fₚ a) ∙ ! (ap f (! (glue (cin j a))) ∙ fₚ a)))))
