...
module ConstrMap5 {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ} (δ : CosDiagMor A F G) where
  ...
  module MapCoher4 {i j : Obj Γ} (g : Hom Γ i j) (a : A) where
    ...
    ψ₂-free-aux3 : {x : P₂} (γ : x == right (cin j (str (G # j) a)))
      {κ : x == x} (ρ : κ == γ ∙ ! γ)
      {z : Colim ForgG} (m₂ : cin j (str (G # j) a) == z) {w : ty (G # j)} (σ : w == str (G # j) a) →
    ...
    ψ₂-free-aux : {x : Colim (ConsDiag Γ A)} (q : cin j a == x) {w₁ w₂ : ty (G # j)} (m₁ : w₁ == str (G # j) a)
      {z : Colim ForgG} (m₂ : cin j w₁ == z) (σ : w₂ == str (G # j) a) →
    ...
    ψ₂-free : (q : cin j a == cin i a) {e : ty (F # j)} (s : e == str (F # j) a) {x₁ x₂ : ty (G # i)} (d : x₁ == x₂)
      (m : fst (G <#> g) x₂ == str (G # j) a) →
    ...
    ψ₂-red-aux3 : {x : P₂} (p : x == right (cin j (str (G # j) a))) →
      ap ! (∙-unit-r p) ∙ ! (ap (λ q → q) (∙-unit-r (! p))) ∙ idp
        ==
      ↯ (ψ₂-free-aux3 p (! (!-inv-r p)) idp idp)
    ψ₂-red-aux3 idp = idp

    ψ₂-red-aux2 : {x : Colim (ConsDiag Γ A)} (q : cin j a == x) {w : ty (G # j)} (σ : w == str (G # j) a)
      {m₂ : cin j (str (G # j) a) == ψ₂ x} (τ : ap ψ₂ q == m₂) → 
    ...
    ψ₂-red-aux : {w₁ w₂ : ty (G # j)} (m₁ : w₁ == str (G # j) a) (m₂ : cin j w₁ == ψ₂ (cin i a))
      (σ : w₂ == str (G # j) a) (τ : ap ψ₂ (cglue g a) == ! (ap (cin j) m₁) ∙ m₂) →
    ...
    abstract
      ψ₂-red : {e : ty (F # j)} (s : e == str (F # j) a) {x : ty (G # i)} (d : x == str (G # i) a) →
        ap !
      ...
      ψ₂-red idp idp = ψ₂-red-aux (snd (G <#> g) a) (cglue g (str (G # i) a)) (snd (nat δ j) a) (ψ₂-βr g a)
