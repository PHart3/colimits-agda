...
module ConstrMap9 {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ} (δ : CosDiagMor A F G) where
  ...
  module MapCoher8 {i j : Obj Γ} (g : Hom Γ i j) (a : A) where     
    ...
    𝕕-red-aux : {x : Colim ForgF} (c : cin j (fst (F <#> g) (str (F # i) a)) == x)
      (d₂ :
        right (cin j (fst (nat δ j) (fst (F <#> g) (str (F # i) a))))
          ==
        right (cin j (fst (nat δ j) (fst (F <#> g) (str (F # i) a)))))
      (ρ : idp == d₂ ∙ idp) →
    ...
    𝕕-red : {z : ty (F # j)} (d₄ : fst (F <#> g) (str (F # i) a) == z)
      ...
      idp ◃∙
      ap2-!-!-!-rid2 𝕕₀ d₄ (cglue g (str (F # i) a)) d₃ ◃∙
      ap (λ p → ! (! (ap (right ∘ cin j ∘ fst (nat δ j)) d₄) ∙ ap (𝕕₀ ∘ right) (cglue g (str (F # i) a))) ∙ ! p) ρ ◃∎
        =ₛ
      ap (λ p →
           ! ((ap (right ∘ cin j ∘ fst (nat δ j)) d₄ ∙
             ((ap 𝕕₀ (! d₃) ∙ idp) ∙ ! (ap 𝕕₀ (! d₃) ∙ idp)) ∙ ! (ap (right ∘ δ₀ ∘ cin j) d₄)) ∙ p) ∙
           ap (right ∘ cin j ∘ fst (nat δ j)) d₄ ∙
           ap (right ∘ cin j) d₁ ∙ ! d₂) (∘-ap 𝕕₀ right (cglue g (str (F # i) a))) ◃∙
      ↯ (long-coher3 (right {d = SpCos₂}) (cin j) d₁ (ap (right ∘ cin j ∘ fst (nat δ j)) d₄)
        (ap 𝕕₀ (! d₃) ∙ idp) d₂ (ap (right ∘ δ₀) (cglue g (str (F # i) a)))) ◃∎
    𝕕-red idp idp idp d₂ ρ = =ₛ-in (𝕕-red-aux (cglue g (str (F # i) a)) d₂ ρ)
    ...
    δ₀-red-aux : {z : ty (F # j)} (s : z == str (F # j) a)
    ...
    δ₀-red :
      {κ :
        cin j (fst (nat δ j) (fst (F <#> g) (str (F # i) a)))
          ==
        cin i (fst (nat δ i) (str (F # i) a))}
      (τ : ap δ₀ (cglue g (str (F # i) a)) == κ) →
      ...
      ↯ (δ₀-free κ (snd (nat δ j) a) (glue (cin j a))
        (ap (right ∘ cin j ∘ fst (nat δ j)) (snd (F <#> g) a))) ◃∎
    δ₀-red idp = =ₛ-in (δ₀-red-aux (snd (F <#> g) a) (cglue g (str (F # i) a)))
