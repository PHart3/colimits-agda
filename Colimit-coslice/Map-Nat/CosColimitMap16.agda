...
module _ {ℓv ℓe ℓ ℓF ℓG} {Γ : Graph ℓv ℓe} {A : Type ℓ} {F : CosDiag ℓF ℓ A Γ} {G : CosDiag ℓG ℓ A Γ}
    ...
    abstract
      ...
      fib-coher : 
        ...
        ap (λ p → ! (p ∙ ap 𝕕₀ (ap right (cglue g (str (F # i) a)))) ∙
             ...
             ! (glue (cin j a))) (hmtpy-nat-rev (λ _ → idp) (snd (F <#> g) a) (ap 𝕕₀ (! (glue (cin j a))) ∙ idp)) ◃∙
        ap (λ p →
              ! ((ap (right ∘ cin j ∘ (fst (nat δ j))) (snd (F <#> g) a) ∙
                (p ∙ ! (ap 𝕕₀ (! (glue (cin j a))) ∙ idp)) ∙
                ! (ap (𝕕₀ ∘ right ∘ cin j) (snd (F <#> g) a))) ∙
                ap 𝕕₀ (ap right (cglue g (str (F # i) a)))) ∙
              ap (right ∘ cin j ∘ (fst (nat δ j))) (snd (F <#> g) a) ∙
              ap (right ∘ cin j) (snd (nat δ j) a) ∙
              ! (glue (cin j a)))
          (ap-inv-rid 𝕕₀ (glue (cin j a)) ∙
          ap ! (𝕕-βr (cin j a)) ∙
          ...
          (!-ap-ap-∘-ap-∙ 𝕕₀ (right ∘ cin j) (snd (F <#> g) a) (ap right (cglue g (str (F # i) a))) ∙
            ap (λ p → p ∙ idp)
              (ap (ap 𝕕₀)
                ...
                E₃ (λ x → ! (glue x)) (cglue g a) (ψ₁-βr g a) (λ _ → idp) ∙
                ∙-unit-r (! (glue (cin i a)))))) ◃∙
        ...
        ap (λ p → ap (right ∘ cin i) (snd (nat δ i) a) ∙ p)
          (! (ap (λ p →
                   ! (ap right (! (ap (cin j) (snd (G <#> g) a)) ∙ cglue g (str (G # i) a))) ∙ ! (glue (cin j a)) ∙ p)
               (ap (ap left) (id-βr g a)))) ◃∙
        ap (λ p → ap (right ∘ cin i) (snd (nat δ i) a) ∙ p)
          (E₃ (λ x → ! (glue x)) (cglue g a) (ψ₂-βr g a) (λ _ → idp)) ◃∙
  ...
  -- 𝕕 belongs to the fiber over over CC-from-diagmap
