...
module _ {ℓv ℓe : ULevel} (G : Graph ℓv ℓe)
  ...
  module _ (K₁ : Cocone Δ T) where
    ...
    abstract
      coc-contr : is-contr (Σ (Cocone Δ T) (λ K₂ → K₁ =-coc K₂))
    ...
    coc-ind : ∀ {k} (P : (K₂ : Cocone Δ T) → (K₁ =-coc K₂ → Type k))
      → P K₁ (=-coc-id K₁) → {K₂ : Cocone Δ T} (e : K₁ =-coc K₂) → P K₂ e
    coc-ind P = ID-ind-map P coc-contr

  coc-to-== : {K₁ K₂ : Cocone Δ T} → K₁ =-coc K₂ → K₁ == K₂
