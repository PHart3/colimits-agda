...
module lib.wild-cats.Diag-ty-WC where
  ...
  module _ {ℓv ℓe} where
    ...
    obj const-diag-ty-WF A = Δ-wc (λ _ → A) (λ f → idf A)
    arr const-diag-ty-WF f = map-diag-ty (λ _ → f) (λ _ _ → idp)
    id const-diag-ty-WF A = dmap-ty-to-== ((λ _ _ → idp) , (λ _ _ → idp))
    comp const-diag-ty-WF f g = dmap-ty-to-== ((λ _ _ → idp) , (λ _ _ → idp))
