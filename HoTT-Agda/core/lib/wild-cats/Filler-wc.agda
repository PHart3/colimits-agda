...
module _ {i j} {C : WildCat {i} {j}} where
  ...
  llp-wc : ∀ {k} (H : ∀ {a b} → hom C a b → -1 -Type k) {a b : ob C} (l : hom C a b) → Type (lmax i j)
  llp-wc H {a} {b} l = ∀ {c d} {r : hom C c d} (f : hom C a c) (g : hom C b d) (S : ⟦ C ⟧ g ◻ l == ⟦ C ⟧ r ◻ f) →
    is-contr (Fill-wc f g S)
