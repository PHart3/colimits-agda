...
module homotopy.ColimAdjointConst {ℓv ℓe : ULevel} (Γ : Graph ℓv ℓe) where

  open Map-diag-ty

  coc-ty-post-comp : ∀ {ℓ ℓ' ℓ''} {X : Type ℓ} {Y : Type ℓ'} {Δ : Diagram Γ (Type-wc ℓ'')}
  ...
  module CCAdj (ℓ : ULevel) where

    ColimConst-ty-Adj : Adjunction (ColimFunctor {i = lmax (lmax ℓv ℓe) ℓ}) (const-diag-ty-WF {i = lmax (lmax ℓ ℓv) ℓe} Γ)
