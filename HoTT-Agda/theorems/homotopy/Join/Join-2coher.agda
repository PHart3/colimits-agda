...
module JLA-2-coher-crd {i₀} (A : Ptd i₀) {i₁ i₂ i₃ i₄} {X : Ptd i₁} {Y : Ptd i₂} {Z : Ptd i₃} {W : Ptd i₄} where
  ...
  abstract
    2-coher-Join-crd : (h₁ : A ⊙* X ⊙→ Y) (h₂ : Z ⊙→ X) (h₃ : W ⊙→ Z) →
    ...
    2-coher-Join-crd (f₁ , idp) (f₂ , idp) (f₃ , idp) = ∼⊙homog∼ ⊙→-homog-str-⊙Ωcod _ (λ w →
      ⊙∘-conv-tri-∙! _ _ _ _ ∙
      ap ⊙-crd∼-to-== (⊙→∼-to-== (∼⊙Ωhomog∼ λ a →
        (2-coher-Join-crd-gen f₁ (jmap (λ x → x) f₂) (jmap (λ x → x) f₃) (jmap (λ x → x) (f₂ ∘ f₃))
    ...
    2-coher-Join h₁ h₂ h₃ = =ₛ-out $
      ...
      ⊙-crd∼-to-==
        ...
        ap-crd-into
          ...
          ⊙∘-post h₁ (!-⊙∼ (jmap⊙-un-∘ h₂ h₃))) ∙⊙∼
        !-⊙∼ (JLA-nat-dom-⊙∼ (h₂ ⊙∘ h₃) h₁)) ◃∎
        =ₛ₁⟨ ap ⊙-crd∼-to-== (⊙→∼-to-== (2-coher-Join-crd h₁ h₂ h₃)) ⟩
