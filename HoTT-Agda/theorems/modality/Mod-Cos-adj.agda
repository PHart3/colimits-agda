...
module Mod-Adj {ℓ j} (μ : Modality ℓ) (A : Type j) where
  ...
  id Mod-cos-fctr X = UndHom∼-to-== (Mod-cos-fmap-id μ A X)
  comp Mod-cos-fctr f g = UndHom∼-to-== (Mod-cos-fmap-∘ μ A g f)
  ...
  snd (iso Mod-cos-adj {Z} {X}) = is-eq (Mod-cos-hom μ A) (Mod-cos-hom-inv μ A (snd X)) rtrip1 rtrip2
    where abstract
      ...
      rtrip1 f = UndHom∼-to-== ((λ x → ◯-rec-β (snd X) (fst f) x) , λ a → !-inv-l-assoc (◯-rec-β (snd X) (fst f) (str Z a)) (snd f a)) 
      
      rtrip2 : Mod-cos-hom-inv μ A {X = Z} {Y = fst X} (snd X) ∘ Mod-cos-hom μ A ∼ idf _
      rtrip2 f = UndHom∼-to-== ((◯-elim (λ x → =-preserves-local (snd X)) (λ x → ◯-rec-β (snd X) (fst f ∘ η) x)) , λ a →
        ap (λ p → ! p ∙ ◯-rec-β (snd X) (fst f ∘ η) (str Z a) ∙ snd f a)
          (◯-elim-β _ _ (str Z a)) ∙
        !-inv-l-assoc (◯-rec-β (snd X) (fst f ∘ η) (str Z a)) (snd f a))
      
  nat-cod Mod-cos-adj g h = UndHom∼-to-== (Mod-cos-cod μ A g h)
  nat-dom Mod-cos-adj f h = UndHom∼-to-== (Mod-cos-dom μ A f h)
