...
open import lib.wild-cats.Radj-lim
open import Cos-wc
open import SIP-Cos
...
module _ {i} {ℓv ℓe} {G : Graph ℓv ℓe} {Δ : Diagram G (Coslice-wc A (lmax i j))} {X : Coslice (lmax i j) j A}
  {K : Cone-wc Δ X} (lim : is-lim-wc K) where
    ...
    Forg-cos-lim-pres Z = ∼-preserves-equiv
      (λ f₁ → con-to-== ((λ _ → idp) , λ {t} f → 
        ap (ap (λ x₁ x₂ → fst x₁ (inl x₂)))
          (ap (λ p → ! (coe p idp) ∙ ap _ (Cone-wc.tri K f))
            (ap (ap (λ (g , _) → (λ x →
               fst (D₁ Δ f) (fst (Cone-wc.leg K t) (cos-map-promote A f₁ x))) ,
               (λ a → ap (fst (D₁ Δ f)) (snd (Cone-wc.leg K t) a) ∙ snd (D₁ Δ f) a) == g))
            (!-inv-l (snd (has-level-apply UndFunHomContr)
              (((λ x → fst (D₁ Δ f) (fst (Cone-wc.leg K t) (cos-map-promote A f₁ x))) ,
                (λ a → ap (fst (D₁ Δ f)) (snd (Cone-wc.leg K t) a) ∙ snd (D₁ Δ f) a)) ,
               (λ _ → idp) , (λ _ → idp)))))) ∙ aux {g = f₁} {x = t} {f = f} (Cone-wc.tri K f)))
      (snd (hom-to-lim-R lim (free-forg-adj-cos A {i})))
      where
        aux : {g : Z → ty X} {x y : Obj G} {f : Hom G x y} {c₁ c₂ : X *→ D₀ Δ y} (q : c₁ == c₂) →
          ap (λ x₁ x₂ → fst {B = λ k → (a : A) → k (inr a) == str (D₀ Δ y) a} x₁ (inl {B = A} x₂))
            (ap (λ v →
